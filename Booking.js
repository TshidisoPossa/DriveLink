/* ═══════════════════════════════════════
   ...
   — loads car data from localStorage / URL
   — renders calendar
   — calculates cost
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Try localStorage first (set by dashboard bookCar())
  ...
  // Fallback to CAR_DATABASE from carData.js
  if (!car && carId && typeof CAR_DATABASE !== 'undefined') {
    car = CAR_DATABASE[carId] || null;
  }

  // Last resort — show placeholder so page does not break
  if (!car) {
    car = {
      id:           carId || 1,
      name:         'Vehicle',
